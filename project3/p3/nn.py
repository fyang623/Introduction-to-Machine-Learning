...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-2, -1, -1):
            ...
            ### YOUR CODE HERE ###
            grad = np.multiply(self.activation_function.backprop_grad(np.matmul(ws[idx],D_stack[idx])+bs[idx]),np.matmul(ws[idx+1].transpose(),grad))
...
class SquaredLoss(LossFunction):
    def loss(self, Y, Yhat):
        ...
        return 1 / (Y.shape[1]*2) * np.linalg.norm((Y - Yhat))**2
