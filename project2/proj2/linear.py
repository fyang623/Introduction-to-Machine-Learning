...
class SquaredLoss(LossFunction):
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        #print(Y.shape)
        #print(X.shape)
        #print("sum_raw = " + str((Y - Yhat) * X.T))
        #print("sum = " + str(- sum((Y - Yhat) * X.T, axis=1)))
...
class LogisticLoss(LossFunction):
    ...
    def loss(self, Y, Yhat):
        ...
        ### TODO: YOUR CODE HERE
        return sum([log(1 + exp(- Y[i] * Yhat[i])) for i in range(len(Y))])
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        for i in range(len(Y)):
            g += 1 /(1 + math.exp(- Y[i] * Yhat[i])) * math.exp(- Y[i] * Yhat[i]) * (- Y[i] * X[i])
...
class LinearClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        def func(w):
            ...
            #print(X.shape)
            #print(w.shape)
            Yhat = X.dot(w)   ### TODO: YOUR CODE HERE
