...
class DT(BinaryClassifier):
    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            self.label = 1 if Y.sum() >= 0 else -1     ### TODO: YOUR CODE HERE
            ...
            for d in range(D):
                ...
                ### TODO: YOUR CODE HERE
                leftCounter = Counter(leftY)
                rightCounter = Counter(rightY)
                leftError = min(leftCounter[1], leftCounter[-1])
                rightError = min(rightCounter[1], rightCounter[-1])
            ...
            self.left.trainDT(leftX, leftY, maxDepth - 1, used.copy())
            self.right.trainDT(rightX, rightY, maxDepth - 1, used.copy())
...
#     runClassifier.trainTestSet(DT({'maxDepth': i}), datasets.SentimentData)

curve = runClassifier.learningCurveSet(DT({'maxDepth': 9}), datasets.SentimentData)
